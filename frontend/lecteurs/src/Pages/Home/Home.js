...
const Home = () => {
  ...
  const searchReview = (values, { setSubmitting }) => {
    ...
    if (values.searchInput !== "") {
      const filteredData = mock.filter(item => {
        return (
          item.author.toLowerCase().includes(values.searchInput.toLowerCase()) ||
          item.name.toLowerCase().includes(values.searchInput.toLowerCase())
        );
      });
  ...
  return (
    ...
    alignItems: 'center', justifyContent: 'space-between'}}>
      <Search searchReview={searchReview} myInput={setSearchInput} />
      </div>
      <Routes>
          <Route path="/" element={<Feed/>}/>
          <Route path="/search" element={<SearchPage mock={results}/>}/>
        </Routes>
    </section>
  )
}
